...
if(!isset($_SESSION['username'])) {
    ...
    $conn=mysqli_connect('localhost','root','','capstone');       
            ...
            $sql1 = "SELECT * FROM commute WHERE Enum = '$Enum'";
...
<html lang="en">
                                    ...
                                    <tbody>
                                        ...
                                        if ($result1->num_rows > 0) {
                                            while ($row = $result1->fetch_assoc()) {
                                                echo "<tr>";
                                                $date = substr($row['inTime'], 0, 10); // inTime에서 날짜 부분 추출
                                                echo "<td>" . $date . "</td>";
                                            ...
                                            echo "<tr><td colspan='6'>정보가 없습니다.</td></tr>";
                                        }
    ...
    <script>
        ...
        // 매출 데이터 가져오기
        $sql = "SELECT SUBSTR(month, 6, 2) AS month, SUM(base_sal) AS total_salary FROM salary WHERE Enum = 2 GROUP BY SUBSTR(month, 6, 2) ORDER BY month;";
        $result = $conn->query($sql);
